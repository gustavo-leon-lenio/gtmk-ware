...
func set_up_rng(_seed = null) -> int:
	var dseed = _seed
	if !dseed:
		dseed = OS.get_unix_time() << 35  # 35 is an arbitrary number but produces enough entropy
...
func table_create(id, rollable_table: Array) -> void:
	tables[id] = {
		active = rollable_table.duplicate(true), original = rollable_table.duplicate(true)
	}
